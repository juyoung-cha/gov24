...
class BlogWriter:
    ...
    def _create_prompt(self, title: str, content: str, dept: str, images: list = None, recent_posts: list = None) -> str:
        ...
        prompt = f"""
당신은 대한민국 국민들의 실생활에 꼭 필요한 핵심 정책과 복지 혜택을 알기 쉽게 분석하여 전달하는 전문 정책 에디터입니다.
...
2. 페르소나 및 문체:
   - 신뢰감 있고 따뜻하며 친절한 전문 분석 톤 ('~입니다', '~하시죠?' 체).
   - 독자의 현실적인 고민(물가, 주거비, 육아, 자녀 교육, 재취업, 노후, 세금)을 짚어주며 깊은 공감 형성.

3. ⚠️ 작성자 이름/가상 인물명 표기 절대 금지 (극도로 중요):
   - 본문 서두, 본문 중, 맺음말 어디에도 가상의 인물 이름(예: '칼럼니스트 OOO', '에디터 OOO', '기자 OOO', 'OOO 드림' 등 특정 개인 성명)을 절대로 작성하지 마세요.
   - 인사말은 "안녕하세요! 오늘도 꼭 필요한 생활 정책과 실속 혜택을 전해드립니다." 또는 "반갑습니다! 여러분의 일상에 실질적인 힘이 되는 정책 소식입니다."처럼 개인 이름 없이 자연스럽고 친근하게 시작하세요.
   - 맺음말 역시 개인 이름 서명 없이 "여러분의 안정되고 행복한 일상을 언제나 응원합니다.", "궁금한 점이 있으시다면 언제든 편하게 확인해 보시기 바랍니다."처럼 정중하고 따뜻하게 마무리하세요.

4. 필수 본문 구조 (3,000자 이상의 깊이 있는 고품질 장문):
   ① [따뜻한 인사 및 일상 공감 서두]: (개인 이름 없이) 최근 독자들의 현실 고민 공감 + 이번 정책이 주는 핵심 혜택 3줄 요약
   ...
   ⑤ [따뜻한 응원과 마무리 메시지]: (개인 이름 없이) 삶을 격려하는 진정성 있는 맺음말
...
"""
    ...
    def _sanitize_author_mentions(self, content: str) -> str:
        """가상 인물 이름(예: 칼럼니스트 김철수, 에디터 홍길동 등) 잔여 패턴 정제"""
        if not content:
            return ""
            
        sanitized = content
        
        # 1. '..., 칼럼니스트/에디터 홍길동입니다.' -> '...입니다.'
        sanitized = re.sub(
            r'[,，]\s*(?:20년\s*(?:경력의|차)?\s*)?(?:시니어\s*)?(?:베테랑\s*)?(?:정책\s*)?(?:칼럼니스트|에디터|저널리스트|기자|전문가)\s+[가-힣]{2,4}\s*입니다',
            '입니다',
            sanitized
        )
        
        # 2. '칼럼니스트/에디터 홍길동입니다.' -> '정책 정보 에디터입니다.'
        sanitized = re.sub(
            r'(?:20년\s*(?:경력의|차)?\s*)?(?:시니어\s*)?(?:베테랑\s*)?(?:정책\s*)?(?:칼럼니스트|에디터|저널리스트|기자|전문가)\s+[가-힣]{2,4}\s*입니다',
            '정책 정보를 전해드리는 에디터입니다',
            sanitized
        )
        
        # 3. '홍길동 칼럼니스트/에디터입니다.' -> '에디터입니다.'
        sanitized = re.sub(
            r'[가-힣]{2,4}\s+(?:칼럼니스트|에디터|저널리스트|기자)\s*입니다',
            '정책 에디터입니다',
            sanitized
        )
        
        # 4. '홍길동 드림', '홍길동 올림', '작성자: 홍길동', '에디터: 홍길동'
        sanitized = re.sub(r'[가-힣]{2,4}\s*(?:드림|올림)(?=\s*<|\s*$)', '', sanitized)
        sanitized = re.sub(r'(?:작성자|글쓴이|에디터|칼럼니스트|기자)\s*:\s*[가-힣]{2,4}', '', sanitized)
    ...
    def write_post(self, title: str, content: str, dept: str, url: str = None, images: list = None, recent_posts: list = None) -> dict:
        ...
        try:
            ...
            if post_data and post_data.get("content"):
                ...
                post_data["content"] = self._sanitize_author_mentions(post_data["content"])
