...
class ContentScraper:
    ...
    def _extract_images(self, soup: BeautifulSoup, url: str) -> list:
        ...
        base_url = "https://www.korea.kr" if "korea.kr" in url else \
                   ...
                   "https://www.mohw.go.kr" if "mohw.go.kr" in url else ""
        ...
        for img in content_area.find_all("img"):
            ...
            if src.startswith("http") and src not in images:
                ...
                ignore_keywords = [
                    ...
                    # [NEW] 서울시 미디어허브 공통 이미지 필터
                    "character", "캐릭터", "mascot", "banner_",
                    "top_banner", "bnr_", "bg_", "bullet",
                    "arrow", "dot_", "line_", "bul_",
                    # [NEW] 장식/레이아웃 이미지 필터
                    "blank.gif", "blank.png", "transparent",
                    "1x1", "pixel", "tracking", "counter",
                    "ad_", "advertisement", "popup",
                    # [NEW] 공통 UI 이미지 필터
                    "header_", "footer_", "menu_", "nav_",
                    "thumbnail_default", "no_image", "noimage",
                    "default_thumb", "img_default"
                ]
                if any(x in src.lower() for x in ignore_keywords):
                    continue
                
                # [NEW] 이미지 크기 필터 (width/height 속성 체크)
                width = img.get("width", "")
                height = img.get("height", "")
                try:
                    w = int(str(width).replace("px", "")) if width else 999
                    h = int(str(height).replace("px", "")) if height else 999
                    if w < 100 or h < 100:
                        continue  # 100px 미만은 아이콘/장식으로 판단
                except (ValueError, TypeError):
                    pass
                
                # [NEW] 파일 확장자 필터 (gif 아이콘 제외, 단 큰 gif는 허용)
                if src.lower().endswith('.gif') and ('icon' in src.lower() or 'btn' in src.lower()):
                    continue
