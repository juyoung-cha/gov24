...
class BlogWriter:
    ...
    def _create_prompt(self, title: str, content: str, dept: str, images: list = None) -> str:
        ...
        # 이미지 정보 추가
        image_prompt = ""
        no_image_warning = "원문에 이미지가 있더라도, 현재 제공된 이미지 URL 리스트가 비어있으므로 본문에 <img> 태그를 절대로 포함하지 마세요. 텍스트와 표(table)만으로 깔끔하게 구성하세요."
        if images:
            image_prompt = "\n**이미지 활용 지침:**\n"
...
**⚠️ 절대 금지 사항: 출력 시 ```html 또는 ```, ''' 같은 마크다운 코드 블록 형식을 절대로 사용하지 마세요. 오직 순수 텍스트와 HTML 본문만 답변하세요.**
...
"""
    ...
    def _parse_response(self, response_text: str, original_title: str, images: list = None) -> Dict:
        ...
        # [PRE-FIX] 응답 전체에서 마크다운 코드 블록 기호 제거
        import re
        response_text = re.sub(r"```[a-zA-Z]*\n", "", response_text) # 시작 태그 (```html 등)
        response_text = response_text.replace("```", "") # 끝 태그 또는 잔재
        response_text = re.sub(r"'''[a-zA-Z]*\n", "", response_text)
        ...
        for line in lines:
            ...
            # AI가 가끔 "제목: ```html" 처럼 출력하는 경우 대비
            if line.startswith("제목:"):
                blog_title = line.replace("제목:", "").replace("```html", "").replace("```", "").strip()
                current_section = None
