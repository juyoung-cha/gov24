...
class BloggerPoster:
    ...
    def _build_jsonld(self, title: str, content: str, meta_description: str = "", 
                      dept: str = "") -> str:
        """
        JSON-LD 구조화 데이터 생성 (Article + FAQPage)
        Google Rich Results 노출을 위한 Schema.org 마크업
        """
        ...
        article_schema = {
            ...
            "@type": "Article",
            "headline": title[:110],  # Google 권장 110자 이내
            "description": meta_description or title,
            "author": {
                "@type": "Organization",
                "name": "Stories of people"
            },
            "publisher": {
                "@type": "Organization",
                "name": "Stories of people"
        ...
        jsonld_html = f'<script type="application/ld+json">{json_module.dumps(article_schema, ensure_ascii=False)}</script>\n'
        
        # FAQ 스키마 (본문에 Q&A 패턴이 있는 경우)
        # 새 형식: <p><strong>Q. 질문</strong></p><p>A. 답변</p>
        # 기존 형식도 호환: <summary>Q. 질문</summary><p>A. 답변</p>
        faq_pattern = re.findall(
            r'<(?:strong|summary)>\s*Q\.\s*(.*?)</(?:strong|summary)>\s*</p>\s*<p>\s*A\.\s*(.*?)</p>',
        ...
        if faq_pattern:
            ...
            jsonld_html += f'<script type="application/ld+json">{json_module.dumps(faq_schema, ensure_ascii=False)}</script>\n'
            logger.info(f"FAQ 구조화 데이터 추가: {len(faq_entities)}개 항목")
        
        return jsonld_html

    def post(self, title: str, content: str, labels: List[str] = None, 
             is_draft: bool = False, meta_description: str = "", 
             dept: str = "") -> Optional[str]:
        """
        블로그 글 포스팅 (SEO v2: JSON-LD + meta description 삽입)
        ...
        Args:
            ...
            meta_description: SEO용 메타 설명 (150자 이내)
            dept: 부처/기관명
